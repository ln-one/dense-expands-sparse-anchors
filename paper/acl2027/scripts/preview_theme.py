...
def vectorize_tint_images(svg):
    """Turn the three flat diagnostic-cell fills into themeable SVG rectangles."""
    import base64
    import pymupdf
    ns = '{http://www.w3.org/2000/svg}'
    palette = {(244,244,244): '#bbbbbb', (214,238,235): '#009988',
               (247,222,216): '#cc3311'}
    for el in list(svg.iter(ns+'image')):
        href = el.get('{http://www.w3.org/1999/xlink}href', el.get('href',''))
        if not href.startswith('data:image/png;base64,'): continue
        pix = pymupdf.Pixmap(base64.b64decode(href.split(',',1)[1]))
        if pix.n != 3: continue
        data = pix.samples
        colors = {tuple(data[i:i+3]) for i in range(0,len(data),3)}
        if not colors.issubset(palette): continue
        rects = []; active = {}
        for y in range(pix.height):
            runs = []; x = 0
            while x < pix.width:
                color = tuple(data[3*(y*pix.width+x):3*(y*pix.width+x)+3])
                start = x; x += 1
                while x < pix.width and tuple(data[3*(y*pix.width+x):3*(y*pix.width+x)+3]) == color: x += 1
                runs.append((start,x-start,color))
            next_active = {}
            for run in runs:
                rect = active.pop(run, [run[0],y,run[1],0,run[2]])
                rect[3] += 1; next_active[run] = rect
            rects.extend(active.values()); active = next_active
        rects.extend(active.values())
        attrs = {k:v for k,v in el.attrib.items() if k not in ('width','height','href','{http://www.w3.org/1999/xlink}href')}
        el.clear(); el.tag = ns+'g'; el.attrib.update(attrs)
        for x,y,w,h,color in rects:
            ET.SubElement(el,ns+'rect',{'x':str(x),'y':str(y),'width':str(w),'height':str(h),
                'fill':f'color-mix(in srgb,{palette[color]} 16%,var(--paper))'})

def apply_theme(svg):
    ns = '{http://www.w3.org/2000/svg}'
    vectorize_tint_images(svg)
