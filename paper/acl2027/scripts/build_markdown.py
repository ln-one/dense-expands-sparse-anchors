...
def compile_ast(ast, config, section=None):
    ...
    def visit(node):
        ...
        if kind == "Header":
            content[1][0] = ""
            if (section or {}).get("unnumbered") is True:
...
def compile_markdown(text, config, section=None):
    layout = section or {}
    for title, anchor in layout.get("heading_anchors", {}).items():
        pattern = r"^(#+ " + re.escape(title) + r")$"
        if len(re.findall(pattern, text, re.M)) != 1:
            raise ValueError("Heading changed or ambiguous: " + title)
        text = re.sub(pattern, lambda m: m[1] + "\n\n<!-- anchor: " + anchor + " -->", text, flags=re.M)
    for insertion in layout.get("layout_insertions", []):
        block = "<!-- latex\n" + insertion["latex"] + "\n-->"
        if insertion["before"] is None:
            text = text.rstrip() + "\n\n" + block + "\n"
        else:
            target = insertion["before"]
            if text.count(target) != 1:
                raise ValueError("Layout boundary changed or ambiguous: " + target)
    ...
    for relative, asset in config["assets"].items():
        ...
        with fitz.open(pdf) as doc:
            ...
            if output.suffix == ".svg":
                ...
                # Outlined glyphs avoid missing fonts; neutral surfaces follow the viewing theme.
