...
All generative baselines use the same primary generator. Their generation and
query-integration settings are specified below.
...
This section gives implementation details for the bound-based replay defined
in Section~\ref{sec:quality-access}. The replay starts with empty dense and
sparse prefixes. After reading $l_c$
...
Table~\ref{tab:operator-controls} reports matched operator controls. Operator
pairs share references, keep the other channel at Original, and average three
draws within query before inference. The QuDAR control uses the same generated
references as DESA and complete-list RRF over OS, OD, ES, and ED.
...
Against complete uniform QuDAR, DESA's paired effects are $-.00132$
[$-.00470,.00194$] for nDCG and $-.00174$ [$-.00382,.00033$] for Recall
($p_{\mathrm{Holm}}=.4393$ and $.2140$), while reducing total certification
depth by 74.41\% [67.27\%, 80.89\%] ($p_{\mathrm{Holm}}=.0002$).

\begingroup\footnotesize
\noindent\textbf{Confidence-weighting audit.}
Replacing QuDAR's margin weights with $w_c=.25$ while holding its inputs fixed
yields no significant nDCG/Recall difference
($p_{\mathrm{Holm}}=.1182/.5826$); 92.14\%/99.04\% of pooled query--draw
outcomes are unchanged, with dataset means in .2325--.2699. In a
complete-list RRF diagnostic, the weights lower nDCG by .00117
($p_{\mathrm{Holm}}=.0104$), leave Recall unchanged
($p_{\mathrm{Holm}}=.4312$), and raise depth from 5478.4 to 5601.4. Neither
test shows an independent benefit in this setting; the unofficial RRF transfer
does not assess LLM weighting or QuDAR overall.
\endgroup
